...
const obj = {
    ...
    push(trip) { // only allow Trip objects to be added to array
        if (trip instanceof Trip) {
            this._trips.push(trip);
        }
    },

    construct(destination, km, litres) {
        return new Trip(destination, km, litres);
    },

    getTotalKml() { // a read-only property
        let totalKm = 0;
        let totalLitres = 0;
    ...
    toString() {
        ...
        str += "\nCumulative KML: " + this.getTotalKml();
        return str;
    },
};
